...
const BookingList = () => {
  ...
  const formatTime = (start, end) => {
    ...
    return start && end ? `${format(start)} - ${format(end)}` : "N/A";
  };
  ...
  {filteredBookings.map((booking) => {
    ...
    const courseDetails = booking.slot?.courseDetails || "N/A";
    ...
    return (
      <tr key={booking.id || booking.bookingId}>
        <td>{userName}</td>
        <td>{userEmail}</td>
        <td>{`${courseDetails} on ${dateFrom} from ${startTime} to ${endTime}`}</td>
        <td>{bookedAt}</td>
      </tr>
    );
  })}
